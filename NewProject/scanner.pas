...
t : text;
s : Char;
i,z,c_block,c_output,u : integer;
token,Ttoken,masukan,hasil,temp2 : String;
stack,tipe,tempDec : array [1..1000] of string;
...
begin
     ...
     assign(t,'C:\Users\ILYAS\Documents\Scanner\scanner\backup\input2.txt');
     reset(t);
     i := 1;
     while not eof(t) do
     begin
           ...
           begin
                ...
                stack[i] := token;
                        ...
                        if (stack[i] = 'program') or (stack[i] = 'procedure') or (stack[i] = 'function') then
                           tipe[i+1] := 'Identifier header';
...
procedure block2; Forward;
procedure expression; Forward; 
procedure variable; Forward;  
procedure uniqueness(cek : string); Forward;
procedure deklarasi(cek : string); Forward;
...
begin
     ...
     repeat
           if masukan[z] in ['0'..'9'] then
              digit
           else if masukan[z] in ['a'..'z','A'..'Z'] then
               letter;
...
begin
     ...
     else if not(masukan = 'begin') and not(masukan = 'procedure') and not(masukan = 'function') and not(masukan = 'var') then
          begin
               writeln('ERROR : ',masukan,' tidak sesuai');
               delay(4000);halt;
          end;
end;
...
begin
     ...
     else if not(masukan = 'begin') and not(masukan = 'procedure') and not(masukan = 'function') and not(masukan = 'var') and not(masukan = 'const') then
          begin
               writeln('ERROR : ',masukan,' tidak sesuai');
               delay(4000);halt;
          end;
...
begin
     ...
     else if not(masukan = 'begin') and not(masukan = 'procedure') and not(masukan = 'function') then
          begin
               writeln('ERROR : ',masukan,' tidak sesuai');
               delay(4000);halt;
          end;
...
begin
      ...
      else if not(masukan = ';') and not(masukan = '.') and not(masukan = 'else') then
           begin
                writeln;
                writeln('ERROR : ',masukan,' tidak sesuai');
                writeln;
...
procedure statement;
begin
     if (masukan = 'begin') or (masukan = 'if') or (masukan = 'case') or (masukan = 'while') or (masukan = 'repeat') or (masukan = 'for') then
         structuredstmt
     else
         simplestmt;
end;
...
procedure uniqueness(cek : string);
var
   jojo : integer;
   bantu : string;
begin
     ...
     repeat
            ...
            begin
                 writeln(masukan);
                 writeln('ERROR : Identifier ',bantu,' dideklarasikan lebih dari satu kali');
                 delay(40000000);halt;
            end;
            jojo := jojo-1;
     until (bantu = 'program') or (bantu = 'procedure') or (bantu = 'function');
     ...
     if tipe[i] = 'Identifier header' then
        repeat
            bantu := stack[jojo];
            if (cek = bantu) and (tipe[jojo] = 'Identifier header') then
            begin
                 writeln(masukan);
                 writeln('ERROR : Identifier ',bantu,' dideklarasikan lebih dari satu kali');
                 delay(40000000);halt;
...
procedure deklarasi(cek : string);
var
   jojo : integer;
   bantu : string;
begin
     ...
     repeat
            ...
            begin
                ...
                jojo := jojo+1;
     until (bantu = tempDec[1]);
     if not(bantu = cek) then
        writeln('Variabel ',cek,' Belum Terdefinisi');
end;

procedure kodeAntara;
var
   tampung,titik2 : string;
   counter,p : integer;
   post : array[1..1000] of string;
   pri : array[1..10000] of integer;
begin
     p := 1;
     counter := 1;
     writeln;
     writeln('==========PROSES POSTFIX==========');
     repeat
          if stack[counter+1] = ':=' then
          begin
               post[p] := stack[counter];
               p := p+1;
               counter := counter+1;
               titik2 := stack[counter];
               counter := counter+1;
               repeat
                     tampung := stack[counter];
                     if(tampung = '(') then
                     begin
                          counter := counter+1;
                          post[p] := tampung;
